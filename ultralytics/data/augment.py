...
# 在augment.py的Compose类之后添加以下实现（约在文件第20行附近）
# 在 ultralytics/data/augment.py 中修改 ThermalNoise 类
class ThermalNoise:
    def __init__(self, apply_prob=0.5, salt_prob=0.4, gauss_prob=0.6, salt_ratio=0.02, gauss_std=0.03):
    ...
    def __call__(self, data):
        ...
        # 强制转换为Tensor并归一化到[0,1]
        if isinstance(img, np.ndarray):
            img = torch.from_numpy(img).float().permute(2, 0, 1) / 255.0  # HWC->CHW
        
        # 椒盐噪声
        if random.random() < self.salt_prob:
            noise_mask = torch.rand_like(img)
            salt_mask = (noise_mask < self.salt_ratio/2)
            pepper_mask = (noise_mask > 1 - self.salt_ratio/2)
            img = torch.where(salt_mask, 0., img)
            img = torch.where(pepper_mask, 1., img)
        
        # 高斯噪声
        if random.random() < self.gauss_prob:
            gauss_noise = torch.randn_like(img) * self.gauss_std
            img = img + gauss_noise
            img = torch.clamp(img, 0., 1.)
        
        # 转换回原始格式
        if isinstance(data['img'], np.ndarray):
            img = (img.permute(1, 2, 0) * 255.0).byte().numpy()  # CHW->HWC, uint8
